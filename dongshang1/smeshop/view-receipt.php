...
$maxwidth = ($_REQUEST["media"] == "print") ? "100%" : "50%";
	...
	$sql = mysqli_query($connection,"select * from ".$fix."orders where orderno='".$orderno."' and orderstatus!='0' ");
...
";	
	...
	<td width=50% height=80 align=left valign=top>
	ออกโดย: ".$shopname."<br>".$shopaddr." โทร. ".$shoptelno." ".$shopurl."<br>";
	if($dbd!="") {
		echo "หมายเลขประจำตัวผู้เสียภาษี: $dbd";
	}
	echo "
	</td>
	</tr>
	...
	*/
	...
	if($arr[8]!="0000-00-00 00:00:00") {
		echo "<tr><td>วันที่จัดส่งสินค้า: ".thaidate(substr($arr[8],0,10))."</td><td>วิธีจัดส่งสินค้า: ".$arr[9]."</td><td>หมายเลขพัสดุ: ".$arr[10]."</td></tr>";
	}
	...
	<td width=50% height=100 align=left valign=top>
	<br>ผู้ส่ง: ".$shopname."<br>".$shopaddr." โทร. ".$shoptelno." ".$shopurl."
	</td>
	</tr>
	</table><br>";
	...
	echo "<div class='noprint'>
			<table border=0>
			<tr>
			<td><a class='boxshadow boxorose' href=backshopoffice.php?action=vreceipt><i class='fa fa-arrow-circle-left'> ย้อนกลับ</a></td>
			<td><a class='boxshadow boxlightblue' href=download-order.php?act=download&orderno=".$arr['orderno']." target=_blank><i class='fa fa-download'> ดาวน์โหลด</a></td>";
				if ($media=="print") {
